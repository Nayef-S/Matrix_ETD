...
def modified_normalized_adjacency_sparse(adj_matrix):
    # Calculate the degree matrix (sum along rows)
    degree_values = torch.sparse.sum(adj_matrix, dim=1).to_dense()
    degree_matrix_inv_sqrt = torch.diag(1.0 / torch.sqrt(degree_values))

    # Multiply the matrices (Degree^(-1/2) * Adj * Degree^(-1/2))
    adj_matrix_normalized = torch.sparse.mm(
        torch.sparse.mm(degree_matrix_inv_sqrt.to_sparse(), adj_matrix),
        degree_matrix_inv_sqrt.to_sparse()
    )

    # Add an identity matrix to the result
    identity_matrix = torch.eye(adj_matrix.shape[0], dtype=adj_matrix.dtype, device=adj_matrix.device)
    adj_matrix_normalized_with_self_loops = adj_matrix_normalized.to_dense() + identity_matrix

    return adj_matrix_normalized_with_self_loops
...
def main(opt):
    ...
    # adj_unnormalised = graph.get_sparse_adjacency_no_weights(opt['cuda'])
    # adj = modified_normalized_adjacency_sparse(adj_unnormalised)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dt', type=float,default=False,help='dt for ETD method')
